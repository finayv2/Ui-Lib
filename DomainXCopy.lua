...
function LibraryV2.UI(Name)
    local Library
    Library = {
        _UI = game:GetObjects("rbxassetid://7736450747")[1],
...
function LibraryV2:Slider(Name, Min, Max, Start, Callback, Use_Decimals)
    ...
    Slider = {
        ...
        Update = function(...)
			...
			Slider.Asset.Percentage.Text = tostring(New and (Use_Decimals and RoundNumber(((New / Slider.Max) * (Slider.Max - Slider.Min) + Slider.Min), 1) or math.floor((New / Slider.Max) * (Slider.Max - Slider.Min) + Slider.Min) or 0))
...
function LibraryV2:Toggle(State)
